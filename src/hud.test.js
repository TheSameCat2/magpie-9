...
test('menu lists the four entries in order', () => {
  assert.deepEqual(MENU_ITEMS, ['new', 'tutorial', 'help', 'credits'])
})

test('stepMenu moves one row and wraps at both ends', () => {
  assert.equal(stepMenu(0, 1, 4), 1)
  assert.equal(stepMenu(3, 1, 4), 0)
  assert.equal(stepMenu(0, -1, 4), 3)
  assert.equal(stepMenu(2, 0, 4), 2)
})

test('stepMenu clamps an out-of-range index back into the list', () => {
  assert.equal(stepMenu(7, 0, 4), 3)
  assert.equal(stepMenu(-1, 0, 4), 3)
